...
#' points(xx, col = 2)
#' }
occ2sp <- function(x, coord_string = "+proj=longlat +datum=WGS84",
                   just_coords = FALSE) {
  ...
  index <- index[
    (x$latitude[index] < 90) &
      (x$latitude[index] > -90) &
      !is.na(x$latitude[index])]

  spobj <- sp::SpatialPoints(as.matrix(x[index,c('longitude', 'latitude')]),
                             proj4string = sp::CRS(coord_string))

  sp_df <- sp::SpatialPointsDataFrame(
    spobj,
    data = data.frame(x[index, c('name', "prov")]))
